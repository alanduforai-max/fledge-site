...
(function () {
  ...
  const PLATES = {
    ...
    tangency(ctx, w, h, seed) {
      ...
      const anchors = [[w * 0.04, ccy + cr * 0.5], ...pts, [w * 0.96, ccy + cr * 0.62]];
      ...
      for (let s = 0; s < anchors.length - 1; s++) {
        ...
        const dxSeg = x1 - x0;
        const m = Math.max(6, Math.min(30, Math.round(dxSeg / (w * 0.012))));
        const amp = Math.min(h * 0.065, dxSeg * 0.35);
        for (let i = 1; i <= m; i++) {
          const t = i / m;
          const env = Math.sin(Math.PI * t);
          const y = y0 + (y1 - y0) * t
            + (r() - 0.5) * amp * env
            + Math.sin(t * Math.PI * 4 + s * 2) * amp * 0.2 * env;
          ctx.lineTo(x0 + dxSeg * t, y);
        }
      }
      ctx.stroke();
