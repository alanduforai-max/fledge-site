...
(function () {
  ...
  const PLATES = {
    ...
    tangency(ctx, w, h, seed) {
      const ccx = w * 0.5, ccy = h * 0.5, cr = Math.min(w, h) * 0.3;
      const deg = Math.PI / 180;
      // five concyclic points; two chords and their perpendicular bisectors
      // meet at the centre — the construction that proves they share a circle
      const angs = [150, 215, 255, 320, 45].map((d) => d * deg);
      const pts = angs.map((a) => [ccx + Math.cos(a) * cr, ccy + Math.sin(a) * cr]);
      ctx.lineWidth = 0.75; ctx.globalAlpha = 0.2;
      ctx.beginPath(); ctx.arc(ccx, ccy, cr, 0, Math.PI * 2); ctx.stroke();
      const bisect = (p, qq, before, beyond) => {
        const [x1, y1] = pts[p], [x2, y2] = pts[qq];
        ctx.beginPath(); ctx.moveTo(x1, y1); ctx.lineTo(x2, y2); ctx.stroke();
        const mx = (x1 + x2) / 2, my = (y1 + y2) / 2;
        let ux = ccx - mx, uy = ccy - my;
        const ul = Math.hypot(ux, uy) || 1; ux /= ul; uy /= ul;
        ctx.beginPath();
        ctx.moveTo(mx - ux * cr * before, my - uy * cr * before);
        ctx.lineTo(ccx + ux * cr * beyond, ccy + uy * cr * beyond);
        ctx.stroke();
      };
      bisect(1, 3, 0.4, 1.18);
      bisect(0, 2, 0.3, 0.55);
      ctx.beginPath(); ctx.arc(ccx, ccy, 1.5, 0, Math.PI * 2); ctx.fill();
      // markers on the five shared moments
      ctx.globalAlpha = 0.5;
      for (const [px, py] of pts) { ctx.beginPath(); ctx.arc(px, py, 3, 0, Math.PI * 2); ctx.stroke(); }
      // tangent at the third moment: radius drawn faint, right angle ticked,
      // the tangent itself inked full-width
      const aH = angs[2];
      const hx = pts[2][0], hy = pts[2][1];
      const tx = -Math.sin(aH), ty = Math.cos(aH);
      const rx = ccx - hx, ry = ccy - hy;
      const rl = Math.hypot(rx, ry) || 1;
      const rxu = rx / rl, ryu = ry / rl;
      ctx.globalAlpha = 0.2;
      ctx.beginPath(); ctx.moveTo(hx, hy); ctx.lineTo(ccx, ccy); ctx.stroke();
      const q = Math.min(w, h) * 0.035;
      ctx.beginPath();
      ctx.moveTo(hx + tx * q, hy + ty * q);
      ctx.lineTo(hx + tx * q + rxu * q, hy + ty * q + ryu * q);
      ctx.lineTo(hx + rxu * q, hy + ryu * q);
      ctx.stroke();
      ctx.globalAlpha = 0.85; ctx.lineWidth = 1.4;
      const t1 = (w * 0.08 - hx) / tx, t2 = (w * 0.92 - hx) / tx;
      ctx.beginPath();
      ctx.moveTo(hx + tx * t1, hy + ty * t1);
      ctx.lineTo(hx + tx * t2, hy + ty * t2);
      ctx.stroke();
...
<tbody>${rows}</tbody></table></div></details>`;
  ...
  const EMAIL_HTML = `<a href="mailto:hello@fledgetradelab.com">hello@fledgetradelab.com</a>`;

  /* ---------- pages ---------- */
  const PILLARS = [
    { plate: "calibration", seed: 5, title: "Benchmark the trading domain", summary: "Handing a model a market and reading the PnL is not evaluation. We score what a desk must trust: risk, sources, summaries, reasoning that traces to the data, rankings that survive a rerun. Open first; graded datasets follow.", tag: "01 · evaluation as a rating layer" },
    { plate: "ascent", seed: 2, title: "Post-train specialist agents", summary: "The benchmark shows where general models fail; post-training builds narrow agents that pass.", tag: "02 · post-training · agent matrix" },
    { plate: "tangency", seed: 8, title: "Validate live", summary: "An agent matrix that scores well is still a claim. We run it on the lab's own small capital as the evidence engine for AI-native trading.", tag: "03 · live capital · evidence engine" },
  ];
  const PROBLEMS = [
...
<div class="container">
  ...
  <section class="section-tight">
    <figure class="pullquote">
      <blockquote>PnL is a verdict, not an explanation.</blockquote>
      <figcaption>Benchmark methodology, in preparation</figcaption>
    </figure>
  </section>
</div>`;
  }
  ...
  <section class="section-tight">
    <div class="page-head">
      <div class="section-head">
        ...
        <p class="lead" style="max-width:none">Open problems we are working on, and what we have written about them.</p>
      </div>
...
<div class="container">
  <section class="section-tight">
    <div class="section-head">
      ...
      <h1 class="display-2">Engineered for decisions under <em style="font-style:italic;font-weight:300">stochastic reward</em></h1>
    </div>
    <div class="about-grid">
      <div style="display:flex;flex-direction:column;gap:var(--space-6)">
        <p class="lead" style="color:var(--text-body);line-height:var(--leading-body);max-width:none">Trading was the first profession to trust machines with money. It will be the first to trust them with judgment — and no one has built the instrument that says when that trust is earned.</p>
        <p style="font-size:var(--size-body-sm);color:var(--text-secondary)">Code got its proving ground: hundreds of benchmarks, and models that improved because failure was measurable. Trading got PnL — a verdict that arrives late, explains nothing, and forgives every wrong reason. We are building the missing rating layer: evaluations that ask whether a summary was faithful, whether a source is real, whether a ranking survives being run three times, whether a chain of reasoning actually touched the data it cites. Open, so the field can stand on it.</p>
        <p style="font-size:var(--size-body-sm);color:var(--text-secondary)">What the benchmark exposes, post-training repairs. On its graded trajectories we build specialist agents — one for momentum, one for sentiment — each narrow, each admitted only by the evaluation that revealed the gap. Not one model that trades: a matrix of agents, each provably good at one thing.</p>
        <p style="font-size:var(--size-body-sm);color:var(--text-secondary)">And when the matrix earns it, we prove it the only way that counts: live, on our own capital, as evidence. Benchmark. Build. Prove. Each step earns the next; none is skipped.</p>
        <hr class="rule">
  ...
  <section class="section-tight">
    <figure class="pullquote">
      <blockquote>The market is the final exam. We write the ones before it.</blockquote>
...
<div class="container">
  <section class="section-tight">
    <div class="section-head">
      ...
      <p class="lead" style="max-width:none">Thirteen tools for your model \u2014 sourced US-market facts and verification report cards. Free account, one line to connect.</p>
    </div>
    <div class="plan-grid">
      <div class="plan">
        ...
        <ul>
          ...
          <li>Personal token \u00b7 works with any MCP client</li>
          <li>Built-in workflows, updated server-side</li>
        </ul>
        <div class="cta"><a class="btn-cta" href="${SIGNUP}" target="_blank" rel="noopener">Create free account \u2192</a></div>
        <p style="margin:0;font-size:var(--size-caption);color:var(--text-muted)">Google or email \u00b7 takes 30 seconds.</p>
      </div>
      <div class="plan">
        ...
        <div class="cta"><a class="btn-ghost" href="mailto:hello@fledgetradelab.com?subject=Fledge%20Pro%20waitlist">Join the waitlist</a></div>
  ...
  <section class="section" id="quickstart">
    ...
    <p style="font-size:var(--size-body-sm);color:var(--text-secondary);margin-top:var(--space-8)"><strong style="color:var(--text-body)">1.</strong> <a href="${SIGNUP}" target="_blank" rel="noopener">Create your account</a> and copy your token.&nbsp;&nbsp;<strong style="color:var(--text-body)">2.</strong> Add the server:</p>
  ...
  --header "Authorization: Bearer &lt;your-token&gt;"</pre>
    <p style="font-size:var(--size-body-sm);color:var(--text-secondary)">Then type <code>/</code> \u2014 the built-in workflows <code>analyze</code>, <code>verify_answer</code>, <code>whats_new</code> show up automatically. Or just ask: <em>\u201cWhat did Pelosi trade this year?\u201d</em></p>
    <p style="margin-top:var(--space-8)"><a class="btn-ghost" href="#docs">Read the docs \u2192</a>&nbsp;&nbsp;<span style="font-size:var(--size-caption);color:var(--text-muted)">setup for every client, the plugin, tool reference, managing &amp; updates</span></p>
  </section>
  <section class="section-tight">
    <figure class="pullquote">
...
<div class="container">
  <section class="section-tight">
    <div class="section-head">
      ...
      <h1 class="display-2" style="max-width:none">Fledge Grounds documentation</h1>
      <p class="lead" style="max-width:none">Setup for every client, built-in workflows, the full tool reference, and how updates reach you.</p>
    </div>
  </section>

  <section class="section-tight" id="token">
    <div class="section-head"><div class="eyebrow">01</div><h2 class="display-2">Get a token</h2></div>
    <p style="font-size:var(--size-body-sm);color:var(--text-secondary);margin-top:var(--space-6)"><a href="https://mcp.fledgetradelab.com/signup" target="_blank" rel="noopener">Sign up free</a> with Google or email (email sign-up requires a 6-digit verification code). Your personal token looks like <code>fg_\u2026</code> \u2014 treat it like a password. Sign in again anytime to see it. Every call made with your token is attributed to you and logged; what we collect is stated at sign-up and in the <a href="https://mcp.fledgetradelab.com/privacy" target="_blank" rel="noopener">Privacy Policy</a>.</p>
  </section>

  <section class="section-tight" id="clients">
    <div class="section-head"><div class="eyebrow">02</div><h2 class="display-2">Connect your client</h2></div>
    <div class="tool-group"><h3>Claude Code (CLI / VS Code)</h3></div>
    <pre class="cmd">claude mcp add --transport http fledge-grounds \\
  https://mcp.fledgetradelab.com/mcp \\
  --header "Authorization: Bearer &lt;your-token&gt;"</pre>
    <div class="tool-group"><h3>Or as a project file \u00b7 .mcp.json (also Cursor: .cursor/mcp.json)</h3></div>
    <pre class="cmd">{
  "mcpServers": {
    "fledge-grounds": {
      "type": "http",
      "url": "https://mcp.fledgetradelab.com/mcp",
  ...
  <section class="section-tight" id="plugin">
    <div class="section-head"><div class="eyebrow">03</div><h2 class="display-2">Claude Code plugin (recommended)</h2></div>
    <p style="font-size:var(--size-body-sm);color:var(--text-secondary);margin-top:var(--space-6)">The plugin bundles the MCP connection plus an analysis skill that makes your model pull evidence before answering stock questions. Put your token in your shell first:</p>
    <pre class="cmd">export FLEDGE_GROUNDS_TOKEN=fg_xxxxxxxx   # e.g. in ~/.zshrc</pre>
    <p style="font-size:var(--size-body-sm);color:var(--text-secondary)">Then, inside Claude Code:</p>
    <pre class="cmd">/plugin marketplace add alanduforai-max/fledge-plugin
/plugin install fledge@fledge</pre>
  </section>

  <section class="section-tight" id="usage">
    <div class="section-head"><div class="eyebrow">04</div><h2 class="display-2">Using it</h2></div>
    <p style="font-size:var(--size-body-sm);color:var(--text-secondary);margin-top:var(--space-6)">Three workflows ship with the server and appear as slash commands \u2014 type <code>/</code> and look for:</p>
    <div class="tool-group">
      <div class="tool-row"><code>\u2026analyze</code><p>Evidence-grounded analysis of a ticker: facts for, facts against, every claim cited; the model's own read clearly labeled.</p></div>
      <div class="tool-row"><code>\u2026verify_answer</code><p>Fact-checks the previous answer claim-by-claim against primary sources; outputs a verdict table.</p></div>
      <div class="tool-row"><code>\u2026whats_new</code><p>Quick briefing: the most material recent filings, insider moves, and congressional trades, with links.</p></div>
    </div>
    <p style="font-size:var(--size-body-sm);color:var(--text-secondary)">Or just ask in your own words: <em>\u201cEvaluate MU \u2014 facts for and against.\u201d \u00b7 \u201cWhat did Pelosi trade this year?\u201d \u00b7 \u201cDid Berkshire trim AAPL last quarter?\u201d</em></p>
  </section>

  <section class="section-tight" id="managing">
    <div class="section-head"><div class="eyebrow">05</div><h2 class="display-2">Managing &amp; updates</h2></div>
    <div class="tool-group">
      <div class="tool-row"><code>enable / disable</code><p>Claude Code: <code>/mcp</code> shows the connection; <code>/plugin</code> toggles the plugin on or off per project or globally.</p></div>
      <div class="tool-row"><code>remove</code><p><code>claude mcp remove fledge-grounds</code>, or uninstall from <code>/plugin</code>.</p></div>
      <div class="tool-row"><code>updates</code><p>Tools, workflows, and data live server-side \u2014 you get updates automatically on your next request, nothing to reinstall. The plugin shell rarely changes; when it does: <code>/plugin marketplace update fledge</code>.</p></div>
      <div class="tool-row"><code>token</code><p>Sign in again at the <a href="https://mcp.fledgetradelab.com/signup" target="_blank" rel="noopener">signup page</a> to view your token; email <a href="mailto:hello@fledgetradelab.com">hello@fledgetradelab.com</a> to revoke it.</p></div>
    </div>
  </section>

  <section class="section" id="tools">
    <div class="section-head"><div class="eyebrow">06</div><h2 class="display-2">Tool reference</h2>
      <p class="lead">One contract: everything returns who, when, what \u2014 and a link to the primary source. Never a recommendation.</p></div>
    ${GROUPS.map((g) => `
    <div class="tool-group">
      <h3>${g.title}</h3>
  ...
  <section class="section-tight" id="fairuse">
    <div class="section-head"><div class="eyebrow">07</div><h2 class="display-2">Coverage, fair use, legal</h2></div>
    <p style="font-size:var(--size-body-sm);color:var(--text-secondary);margin-top:var(--space-6)">Coverage: ~10,400 SEC-registered US tickers; House STOCK Act disclosures (refreshed corpus); licensed end-of-day market data. Not covered yet: options chains, non-US listings, crypto. Beta: fair use, no SLA. Research data only \u2014 nothing here is investment advice. <a href="https://mcp.fledgetradelab.com/terms" target="_blank" rel="noopener">Terms</a> \u00b7 <a href="https://mcp.fledgetradelab.com/privacy" target="_blank" rel="noopener">Privacy</a></p>
    <p style="margin-top:var(--space-8)"><a class="btn-ghost" href="#connect">\u2190 Back to MCP</a></p>
  </section>
</div>`;
  }
...
<div class="container">
  ...
  function setNav(page) {
    document.querySelectorAll(".site-nav a").forEach((a) => {
      ...
      const active = (r === "research" && (page === "research" || page === "article")) || (r === "connect" && page === "docs") || r === page;
